...
class ViewportRendererThread(threading.Thread):
    ...
    def run(self):
         ...
         while not self.stopped:
             processed_frame = self.process_frame(cap)
        ...
        with self.mp_hands.Hands(
         ...
         if results.multi_hand_landmarks is not None:
             for hand_landmarks in results.multi_hand_landmarks:
                 ...
                 self.mp_drawing.DrawingSpec(color=(0,0,255),thickness=-1, circle_radius=10),
                 self.mp_drawing.DrawingSpec(color=(0,40,255),thickness=4,))
